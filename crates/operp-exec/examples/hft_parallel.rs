...
fn parse_args() -> Cfg {
    ...
    let engines: usize = args.next().and_then(|a| a.parse().ok()).unwrap_or_else(|| {
...
fn main() {
    ...
    println!("READY");
    println!(
        "PIPELINE HFT: {} signers -> 1 executor, {}ms",
        cfg.engines, cfg.run_ms
    );
    ...
    for idx in 0..cfg.engines {
        ...
        gens.push(std::thread::spawn(move || {
            generator(idx, cfg.engines, &c, tx, gt)
        }));
    }
    drop(tx);
