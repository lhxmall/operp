...
function submitData(height, stateRoot, prev) {
  return {
    ...
    // DA commitments (doc 12 §2.4): the rollup submit gates and stores these.
...
async function main() {
  ...
  console.log("1. bind ok — dispute_aa + dispute_fill_aa + dispute_clamp_aa set");
  ...
  const acct = DEP_ACCT;
  // Deposit anchor: standard-base64 unit id the vault receipt keys on. The
  // vault is funded live in 13e so dep_evidence has an exact receipt match.
  ...
  // Insurance zero-account leaf: present in every pre tree so clamp
  // predicates can prove the insurance pre leg (doc 12 §2.1).
  ...
  // NO vault receipt, then kill it with dep_evidence. submitH3 stores:
  // trace=FAKE_TRACE_ROOT, fills=FAKE_OPS_ROOT, ops=OPS_ROOT1,
  // units=UNITS_ROOT, units_set=UNITS_SET_ROOT (submitData defaults).
  ...
  await submitH3(CLAMP_TRACE_ROOT, CLAMP_FILLS_ROOT);
  const clampBase = {
    k: 0,
  ...
  await submitH3(CLAMP_TRACE_H_ROOT, CLAMP_FILLS_ROOT);
